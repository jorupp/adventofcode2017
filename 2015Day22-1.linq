...
abstract class Node<TNode> where TNode:Node<TNode> {
	...
	public string Key { get { return string.Join("-", Keys); } }
}
...
class GameNode : Node<GameNode> {
	...
	public override int CurrentCost { get { return SpentMana; } }
	public override int EstimatedCost { get { return SpentMana + Math.Max(0, (BossHP * 9)); } } // most efficient damage to boss is 9 mana per HP
...
//	var initialState = new GameNode() { PlayerHP = 10, PlayerMana = 250, BossHP = 13, BossDmg = 8, IsPlayerTurn = false }; // player goes first
	...
	var moves = states.Where(i => i.Parent != null && i.Parent.Item2 != null).Select(i => string.Format("{0} - {1} -> {2} - {3} {4} {5}", i.Parent.Item2, i.PlayerMana, i.SpentMana, i.ShieldTurnsLeft, i.PoisonTurnsLeft, i.RechargeTurnsLeft)).Reverse().ToList();
	moves.Dump();
}
