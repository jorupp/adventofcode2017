    ...
    public class Part2 : BasePart
    {
        protected void RunScenario(string title, string input)
        {
            RunScenario(title, () =>
            {
                ...
                var re = new Regex(@"p=<(.*),(.*),(.*)>, v=<(.*),(.*),(.*)>, a=<(.*),(.*),(.*)>");
                var ps = lines.Select((i, ix) =>
                {
                    var m = re.Match(i);
                    var parts = m.Groups.Cast<Group>().Skip(1).Select(ii => long.Parse(ii.Value)).ToArray();
                    return new Particle()
                    {
                        id = ix,
                        Position = new Vector() {X = parts[0], Y = parts[1], Z = parts[2]},
                        Velocity = new Vector() {X = parts[3], Y = parts[4], Z = parts[5]},
                        Acceleration = new Vector() {X = parts[6], Y = parts[7], Z = parts[8]},
                        active = true,
                    };
                }).ToArray();
                ...
                for(var iter =0; iter < 100000; iter++)
                {
                    ...
                    ps.Where(p => p.active).GroupBy(p => p.Position).Where(g => g.Count() > 1).ToList().ForEach(g => g.ToList().ForEach(i =>
                    {
                        //Console.WriteLine($"killed {i.id} at {iter}");
                        i.active = false;
                    }));
                    if (ps.Count(i => i.active) <= 1)
                        break;
                }
                
                Console.WriteLine(ps.Count(i => i.active));
            });
        }

        public override void Run()
        {
            RunScenario("initial", @"p=<-6,0,0>, v=< 3,0,0>, a=< 0,0,0>
p=<-4,0,0>, v=< 2,0,0>, a=< 0,0,0>
p=<-2,0,0>, v=< 1,0,0>, a=< 0,0,0>
p=< 3,0,0>, v=<-1,0,0>, a=< 0,0,0>");
            //return;
            RunScenario("part1", @"p=<1199,-2918,1457>, v=<-13,115,-8>, a=<-7,8,-10>
