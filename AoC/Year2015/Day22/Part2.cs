...
namespace AoC.Year2015.Day22
{
    public abstract class SharedPart : BasePart
    {
        protected void RunScenario(string name, GameNode initialState)
        {
            RunScenario(name, () =>
            {
                var finalState = new GraphSolver.Solver().Evaluate(initialState);
    ...
    public class Part1 : SharedPart
    {
        ...
        {
            RunScenario("Rupp (953)", new GameNode() { PlayerHP = 50, PlayerMana = 500, BossHP = 55, BossDmg = 8, IsPlayerTurn = false }); // Rupp puzzle, should be 953
        }
    }
    ...
    public class Part2 : SharedPart
    {
        public override void Run()
        {
            RunScenario("Rupp (1289)", new Part2GameNode() { PlayerHP = 50, PlayerMana = 500, BossHP = 55, BossDmg = 8, IsPlayerTurn = false }); // Rupp puzzle, should be 1289 - was getting 1295
            RunScenario("Sean (1309)", new Part2GameNode() { PlayerHP = 50, PlayerMana = 500, BossHP = 58, BossDmg = 9, IsPlayerTurn = false }); // Sean puzzle, should be 1309
        }
    }
