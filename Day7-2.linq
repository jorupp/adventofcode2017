...
soxzrwm (83)";
	var lines = input.Split(new [] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
		.Select (i => {
			...
			return new Node() {
				node = parts[0],
				weight = int.Parse(parts[1].Substring(1, parts[1].Length-2)),
				children = parts.Skip(3).Select(ii => ii.TrimEnd(',')).ToArray()
			};
		})
		.ToArray();
	nodes = lines.ToDictionary (i => i.node);
	
	//lines.Dump();

	// run this once to get all the candidates, then start adding extra ones to the set we're dumping out
	//   we're looking for an abnormal node at the lightest place that's off (since that's obviously a child of any heavier places that are off)
	foreach(var node in lines) {
		if(node.children.Length == 0)
			continue;
		var children = node.children.Select(i => weight(nodes[i]));
		if(children.Min() != children.Max() || node.node == "fabacam") {
			children.Dump();
			node.Dump();
//			return;
		}
...
int weight(Node node) {
	return node.weight + node.children.Select(i => weight(nodes[i])).Sum();
}
